...
describe Layout do
  ...
  context 'compute_shrink_factor' do
    it 'computes correct factors' do
      factors = [ 2, 1, 1 / 0.5, 1 / 2.0, 1 / 3.0, 1 / 4.0, 1 / 6.0, 1 / 8.0, 1 / 16.0 ]
      results = [ 1, 1, 1      , 2      , 2      , 4      , 4      , 8      , 8        ]
      factors.map { |f| Layout.compute_shrink_factor(f, f) }.should eq results
    end
  end
end
