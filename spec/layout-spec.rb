...
describe Layout do
  ...
  context 'constraint mode' do
    ...
    context 'max' do
      let(:options) { { :mode => :max, :scale => :both } }
    ...
    context 'pad' do
      let(:options) { { :mode => :pad, :scale => :both } }
    ...
    context 'crop' do
      let(:options) { { :mode => :crop, :scale => :both } }
    ...
    context 'stretch' do
      let(:options) { { :mode => :stretch, :scale => :both } }
    ...
    context 'scale mode' do
      ...
      context 'down' do
        let(:options) { { :mode => :max, :scale => :down } }
    ...
    it 'supports shrink-on-load' do
      info = Layout::ImageInfo.new(400, 200, true, false)
      options = { :width => 20, :height => 10 }
      layout = {
        :load   => Options::Load.new(4),
        :resize => Options::Resize.new(0.2, 0.2)
      }
      Layout.process_info(info, options).should eq layout
    end
